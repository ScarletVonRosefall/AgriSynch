...
class _CalendarPageState extends State<AgriSynchCalendarPage> {
  ...
  Future<void> _loadEvents() async {
    ...
    String? storedData = prefs.getString('calendar_events');
    if (storedData != null) {
      setState(() {
        final decoded = json.decode(storedData);
        _events = {};
        decoded.forEach((key, value) {
          _events[key] = List<Map<String, dynamic>>.from(
            value.map((item) => item is String 
              ? {'title': item, 'type': 'event', 'category': 'Other'}
  ...
  Future<void> _loadTasks() async {
    ...
    if (storedTasks != null) {
      setState(() {
        _tasks = List<Map<String, dynamic>>.from(json.decode(storedTasks));
  ...
  void _syncTasksToCalendar() {
    ...
    _events.forEach((key, events) {
      events.removeWhere((event) => event['type'] == 'task');
    });
  ...
  void _showAddDialog() {
    ...
    final categories = ['Feeding', 'Cleaning', 'Harvesting', 'Maintenance', 'Health Check', 'Other'];
  ...
  Widget build(BuildContext context) {
    final selectedDateKey = _selectedDay?.toIso8601String().split('T')[0];
                              ...
                              return Container(
                                ...
                                child: ListTile(
                                  ...
                                  leading: Container(
                                    ...
                                    height: 48,
                                    decoration: BoxDecoration(
                                      color: _getCategoryColor(event['category'] ?? 'Other').withOpacity(0.1),
                                      borderRadius: BorderRadius.circular(12),
                                    ),
                                    child: Icon(
                                      _getCategoryIcon(event['category'] ?? 'Other'),
                                      color: _getCategoryColor(event['category'] ?? 'Other'),
                                      size: 24,
                                    ),
                                  ),
                                  title: Row(
                                      ...
                                      Expanded(
                                        child: Text(
                                          ...
                                          style: TextStyle(
                                            ...
                                            fontWeight: FontWeight.w600,
                                            fontSize: 16,
                                            color: isTask && event['done'] 
                                                ? Colors.grey 
                                                : Colors.black87,
                                            decoration: isTask && event['done'] 
                                    ...
                                    children: [
                                      ...
                                      Row(
                                        children: [
                                          ...
                                          if (isTask && event['time'] != null && event['time'].isNotEmpty) ...[
                                            const SizedBox(width: 12),
                                            Icon(
                                              Icons.access_time,
                                              size: 14,
                                        ...
                                        Row(
                                          children: [
                                            Container(
                                              ...
                                              height: 8,
                                              decoration: BoxDecoration(
                                                color: event['done'] ? Colors.green : Colors.orange,
